#include "../../headers/Scene Core/CoordinateSystem.h"
...
void CoordinateSystem::drawCoordinateSystem(int bufferWidth, int bufferHeight, const Camera& camera)
{
    shader.useShader();
    ...
    glm::vec3 axisPosition = camera.getCameraPosition() + glm::normalize(camera.getFrontVector()) * 100.f;
    model = glm::translate(model, axisPosition);
    model = glm::scale(model, glm::vec3(200.f));

    shader.setMat4("model", model);

    glViewport(bufferWidth / 50, bufferHeight / 50, bufferWidth / 12, bufferHeight / 12);
