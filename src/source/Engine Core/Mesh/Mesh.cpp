#include "../../../headers/Engine Core/Mesh/Mesh.h"

Mesh::Mesh(GLenum windingOrder) : windingOrder{ windingOrder } {
	objectID = meshCount++;
...
void Mesh::drawMesh(GLenum renderMode) {
	...
	if (drawIndexed) {
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, EBO);
		glDrawElements(renderMode, (GLsizei)indices.size(), GL_UNSIGNED_INT, 0);
		glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, 0);
	}
	else {
		glBindBuffer(GL_ARRAY_BUFFER, VBO);
		glDrawArrays(renderMode, 0, (GLsizei)vertices.size());
		glBindBuffer(GL_ARRAY_BUFFER, 0);
	}
