#include "../../headers/Engine Core/Skybox.h"
...
void Skybox::_generatePrefilterMipmap() {
	...
	for (uint32_t mipLevel = 0; mipLevel < maxMipLevels; mipLevel++) {
		uint32_t mipWidth = static_cast<uint32_t>(PREFILTER_RESOLUTION * pow(0.5f, mipLevel));
		uint32_t mipHeight = static_cast<uint32_t>(PREFILTER_RESOLUTION * pow(0.5f, mipLevel));
